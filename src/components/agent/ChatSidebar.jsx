...
const ChatSidebar = ({ 
    ...
    return (
        <aside className="hidden w-72 flex-col gap-3 lg:flex h-full">
            <div className="flex flex-col gap-3 h-full rounded-3xl border border-chat-border bg-chat-card p-3 backdrop-blur-2xl shadow-xl">
                <button
                    onClick={() => onNewChat()}
                    className="flex items-center gap-2.5 w-full rounded-xl border border-chat-accent/30 bg-chat-accent/10 px-3 py-2.5 text-xs font-bold text-chat-accent transition-all hover:bg-chat-accent/20 active:scale-95"
                >
                    ...
                    {isLoadingSessions ? (
                        ...
                        visibleSessions.map(session => (
                            <div
                                ...
                                className={`group relative flex items-center justify-between gap-2 cursor-pointer rounded-lg p-2.5 transition-all ${currentSessionId === session.id
                            ...
                            >
                                <div className="flex items-center gap-2.5 overflow-hidden">
                                    <MessageSquare className={`h-3.5 w-3.5 shrink-0 ${currentSessionId === session.id ? 'text-chat-accent' : 'opacity-60'}`} />
                ...
                <div className="mt-auto pt-3 border-t border-chat-border">
                    <div className="flex items-center gap-3 p-1.5">
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-chat-accent/30 bg-chat-accent/10 text-chat-accent">
                            <Bot className="h-5 w-5" />
                        </div>
                        <div>
                            <p className="text-[9px] font-bold uppercase tracking-widest text-chat-accent/70">Intelligence</p>
                            <h2 className="text-xs font-black italic tracking-tight text-chat-text-primary">AGENT PRO</h2>
                        </div>
                    </div>
                </div>
            </div>
