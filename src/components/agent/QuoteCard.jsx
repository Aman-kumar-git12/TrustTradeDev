...
const QuoteCard = ({ quote, isBusy, isCompleted, isInitiating, onPay, onCancel }) => {
    return (
        <div className="mt-2 overflow-hidden rounded-2xl border border-chat-accent/30 bg-chat-card/50 backdrop-blur-xl shadow-2xl relative group">
            ...
            <div className="p-3 border-b border-chat-border/50">
                ...
                <div className="space-y-1.5 text-xs">
                    ...
                    <div className="pt-2 mt-2 border-t border-chat-border/50 flex justify-between text-sm font-black text-chat-text-primary">
            ...
            <div className="p-3 bg-chat-accent/5 flex gap-3 relative z-20">
                <button
                    disabled={isBusy || isCompleted || isInitiating}
                    onClick={onPay}
                    className={`flex-1 flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-xs font-black text-white transition-all active:scale-95 shadow-[0_0_20px_rgba(16,185,129,0.3)] disabled:opacity-50 disabled:grayscale ${isCompleted ? 'bg-chat-accent/40' : 'bg-chat-accent hover:opacity-90'}`}
                >
                    {isCompleted ? (
                        <>
                ...
                {!isCompleted && (
                    <button
                        disabled={isBusy}
                        onClick={onCancel}
                        className="rounded-xl bg-transparent border border-chat-border px-3 py-2 text-xs font-bold text-chat-text-secondary transition-all hover:bg-chat-bg hover:text-chat-text-primary active:scale-95 disabled:opacity-50"
                    >
                        Cancel
                    </button>
                )}
